import type { Condition, RuntimeResources, ValueRef } from './types';
import { ExecutionContext } from './execution-context';
import { coerceNumber, getValueByPath, locatorFromDef, resolveVariables } from './step-utils';

export class ConditionEvaluator {
  static resolveValue(ref: ValueRef, ctx: ExecutionContext, resources: RuntimeResources): unknown {
    if (ref === null || typeof ref === 'boolean' || typeof ref === 'number') {
      return ref;
    }
    if (typeof ref === 'string') {
      return resolveVariables(ref, ctx.getSnapshot());
    }
      ...
      case 'jsonPath': {
        ...
        const json =
          typeof body === 'string'
            ...
            : body;
        return getValueByPath(json, ref.path || '$');
      }
      case 'ui': {
        if (!resources.page || !ref.locator) return undefined;
        const locator = locatorFromDef(resources.page, ref.locator);
        const probe = ref.path || 'text';
        if (probe === 'text') return locator.innerText().catch(() => '');
        if (probe === 'value') return locator.inputValue().catch(() => '');
        if (probe === 'visible') return locator.isVisible().catch(() => false);
        if (probe === 'enabled') return locator.isEnabled().catch(() => false);
        if (probe === 'count') return locator.count().catch(() => 0);
  ...
  static evaluate(condition: Condition, ctx: ExecutionContext, resources: RuntimeResources): boolean {
