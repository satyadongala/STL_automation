...
exports.frameworkRoot = void 0;
exports.buildFrameworkScaffold = buildFrameworkScaffold;
exports.inferFeatureFolder = inferFeatureFolder;
exports.inferSpecFileName = inferSpecFileName;
exports.extractSelectorsFromUiSteps = extractSelectorsFromUiSteps;
const ROOT = 'playwright-framework';
exports.frameworkRoot = ROOT;
function buildFrameworkScaffold(ctx) {
    const files = {};
...
function inferFeatureFolder(tc) {
    const blob = `${tc.name} ${tc.path}`.toLowerCase();
    if (tc.testType !== 'UI' && tc.method !== 'UI')
        return 'api';
    if (/logout|sign.?out|log.?out/.test(blob))
        return 'login';
    if (/login|sign.?in|auth/.test(blob))
        return 'login';
    if (/dashboard|home|landing/.test(blob))
        return 'dashboard';
    return 'ui';
}
function inferSpecFileName(tc) {
    const name = tc.name.toLowerCase();
    if (/logout|sign.?out/.test(name))
        return 'logout.spec.ts';
    if (/login|sign.?in/.test(name))
        return 'login.spec.ts';
    if (/dashboard/.test(name))
        return 'dashboard.spec.ts';
    return `${tc.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'test'}.spec.ts`;
}
function extractSelectorsFromUiSteps(uiStepsJson) {
    const selectors = {};
    try {
        const steps = JSON.parse(uiStepsJson || '[]');
        let fillIndex = 0;
...
function generatePlaywrightConfig(ctx) {
...
function dashboardLocatorsTs(ctx) {
